...
func _physics_process_controller(delta: float) -> void:
	...
	if look_dir.length() > 0.001:
		...
		_player_ref.orientation.basis = Basis(q_from.slerp(q_to, delta * _player_ref.ROTATION_SPEED))
	else:
		var q_from = _player_ref.orientation.basis.get_rotation_quaternion()
	...
	var speed := _player_ref.SPEED
	
	if _player_input.run_input:
		speed *= _player_ref.SPEED_MULTIPLIER

	var dir := (camera_basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
	var target_pos := dir * speed
	horizontal_vel = target_pos
